...
export function ShipListComponent() {
  ...
  const sort_fn = (a: string | number, b: string | number) => {
    ...
    if (set_sort() == "Aircraft installed")
      return (
        mst_ships.mst_ships[a_ship.ship_id]?.maxeq.reduce((a, b) => a + b, 0) -
        mst_ships.mst_ships[b_ship.ship_id]?.maxeq.reduce((a, b) => a + b, 0)
      );
  ...
  const filtered_ships = createMemo<{ [key: number]: boolean }>(() => {
    ...
    (set_order()
      ...
      ret[Number(ship_id)] = (() => {
        ...
        if (!check_range("Level", ships.ships[Number(ship_id)].lv))
          return false;
        if (!check_range("Durability", ships.ships[Number(ship_id)].maxhp))
          return false;
        if (!check_range("Firepower", ships.ships[Number(ship_id)].karyoku[0]))
          return false;
        if (!check_range("Torpedo", ships.ships[Number(ship_id)].raisou[0]))
          return false;
        if (!check_range("Anti-Air", ships.ships[Number(ship_id)].taiku[0]))
          return false;
        if (!check_range("Speed", ships.ships[Number(ship_id)].soku))
          return false;
        if (!check_range("Armor", ships.ships[Number(ship_id)].soukou[0]))
          return false;
        if (!check_range("Evasion", ships.ships[Number(ship_id)].kaihi[0]))
          return false;
        if (
          !check_range("Anti-Submarine", ships.ships[Number(ship_id)].taisen[0])
        )
          return false;
        if (!check_range("Luck", ships.ships[Number(ship_id)].lucky[0]))
          return false;
        if (
          !check_range(
            "Aircraft installed",
            ships.ships[Number(ship_id)].slotnum,
          )
        )
          return false;
        if (
          ...
          return false;
        if (!check_range("Range", ships.ships[Number(ship_id)].leng))
          return false;
        return true;
      })();
    });
    ...
    params.forEach((param) => {
      set_range_element[param] = (
        <div class="dropdown dropdown-end">
          ...
          <div
            tabindex="0"
            class="dropdown-content z-[2] card card-compact bg-base-100 z-[1] w-64 shadow rounded-md"
          >
            <div class="card-body">
    ...
    params.forEach((param, param_index) => {
      set_range_element[param] = (
        <div class="dropdown dropdown-end">
          ...
          <div
            tabindex="0"
            class="dropdown-content z-[2] card card-compact bg-base-100 z-[1] w-80 shadow rounded-md"
          >
            <div class="card-body">
              <div class="form-control">
                <label class="label cursor-pointer relative">
  ...
  const [pagination, set_pagination] = createStore<{
    selected: string;
    options: {
      [key: string]: {
        pages: number;
  ...
  createEffect(() => {
    ...
    let pages = Math.ceil(
      Object.keys(filtered_ships()).length / Number(pagination.selected),
    );
    ...
    set_pagination("options", pagination.selected, {
      pages: pages,
  ...
  const show_pagination = (index: number) => {
    ...
    if (
      index < current_page * Number(pagination.selected) &&
      index >= (current_page - 1) * Number(pagination.selected)
    )
      return true;
            ...
            {(ship_id, index) => (
              ...
              >
                <tr class="flex table_hover table_active rounded ml-10 -pl-10">
                  <th
                    class="w-10 flex bg-base-200 z-[1] -ml-10"
                    ...
                    <td class="w-[88px]">
                      {
                        mst_stypes.mst_stypes[
                          mst_ships.mst_ships[
                            ships.ships[Number(ship_id)].ship_id
                          ].stype
                        ].name
                      }
                    </td>
  ...
  return (
    <>
      ...
      >
        ...
        <div class="h-[2px]" />
        <progress
          class="progress progress-info w-screen rounded-none py-0 h-[4px] bg-base-200 -mb-2"
          value={String(progress_value())}
          max="100"
          style={{ position: "sticky", left: "0" }}
        />
        <div class="px-2 py-1 text-xs flex flex-wrap items-center w-screen">
          <div class="flex flex-nowrap items-center">
            ...
            <details class="dropdown">
              ...
              <ul
                tabindex="0"
                class="dropdown-content z-[2] menu menu-xs bg-base-100 rounded-md  shadow flex"
              >
            ...
            <select
              class="select select-sm select-ghost select-bordered z-[10]"
              onChange={(e) =>
                set_pagination("selected", e.currentTarget.value)
              }
          ...
          <Show when={!set_categorize() && pagination.selected != "All"}>
            <div class="flex flex-nowrap items-center mt-2 w-full">
              ...
              >
                <IconChevronDoubleLeft class="h-3 -mt-0.5 " />
              </button>
              <button
              ...
              >
                <IconChevronLeft class="h-3 -mt-0.5" />
              </button>
              <For
              ...
              >
                <IconChevronRight class="h-3 -mt-0.5" />
              </button>
              <button
                class="btn btn-sm btn-ghost mx-0.5 pagination"
                onClick={() =>
              ...
              >
                <IconChevronDoubleRight class="h-3 -mt-0.5" />
              </button>
              <span class="flex-1" />
            </div>
          </Show>
        ...
        <table class="table table-xs">
          <thead>
            <tr class="flex">
              <th
                class="w-10 flex bg-base-200 z-[3]"
                style={{ position: "sticky", left: "0" }}
              >
                <div class="h-lvh w-10 shadow absolute left-0 z-[1]" />
                <div class="dropdown" style={{ "z-index": "3" }}>
                  ...
                  <div
                    tabindex="0"
                    class="dropdown-content z-[2] card card-compact bg-base-100 w-72 shadow rounded-md"
                  >
                    <div class="card-body ">
              ...
              <th class="w-32">
                <div class="dropdown">
                  ...
                  <div
                    tabindex="0"
                    class="dropdown-content z-[2] card card-compact bg-base-100 z-[1] w-72 shadow rounded-md"
                  >
                    <div class="card-body ">
                ...
                <th class="w-[88px]">
                  <div class="dropdown">
                    <div class="indicator">
                      <Show
                        when={
                          Object.values(check_stype).findIndex(
                            (value) => !value,
                          ) != -1
                        }
                      >
                    ...
                    <ul
                      tabindex="0"
                      class="dropdown-content z-[2] menu menu-xs bg-base-100 rounded-md z-[1]  shadow max-h-64 overflow-x-scroll flex"
                    >
                  ...
                  <li>
                    <details>
                      <summary class="ml-10 relative">
                        <div
                          class="w-10 h-6 z-[3] bg-base-200 -ml-[52px] px-0"
                          style={{ position: "sticky", left: "0" }}
                        />
