...
export function AirBasesComponent() {
    ...
    const cond_state = createMemo<JSX.Element[][]>(() => {
        ...
        const set_cond_state = (cond: number): JSX.Element => {
            ...
            else if (cond == 2) cond_state = <IconCautionFill class="h-4 w-4 fill-yellow-500 stroke-2"></IconCautionFill>;
            else if (cond == 3) cond_state = <IconCautionFill class="h-4 w-4 fill-red-500 stroke-2"></IconCautionFill>;
            return cond_state;
        };
        ...
        <>
            <li>
                <details open>
                    ...
                    <ul class="pl-0">
                        <For each={Object.entries(air_bases.bases)} fallback={<div class="text-xs py-2">Loading Air Base Data ...</div>}>
                            {(base, base_index) => <>
                                <li>
                                    <details open>
                                        <summary class="flex">
                                                    ...
                                                    <li class="h-auto">
                                                        <a class="justify-start gap-x-0 gap-y-1 flex flex-wrap">
                                                            <div class="justify-start gap-0 flex ">
                                                                <div class="pl-2 pr-0.5 flex-1 min-w-12 content-center"  style={"overflow-x: clip; overflow-clip-margin: content-box 4px;"}>
                                                                    <div class="w-48">
                                                                        <EquimentComponent slot_id={plane.slotid} name_flag={true} onslot={plane.count ?? 0} />
                                                                    </div>
                                                                </div>
                                                                <div class="divider divider-horizontal mr-0 ml-0 flex-none"></div>
                                                                <div class=" flex-none">
                                                                    <div class="flex justify-center w-8 indicator">
                                                                        <div class="indicator-item indicator-top indicator-end">
                                                                            { cond_state()[base_index()][plane_index()] }
                                                                        </div>
