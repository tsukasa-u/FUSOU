...
export function DebugApi() {
    ...
    const [filecheackSignals, setFilecheackSignals] = createStore<boolean[]>([false, false, false, false, false, false, false, false, false, false]);
    ...
    const fileExistSignals = createMemo<boolean[]>(() => {
        ...
        for (let i = 0; i < filecheackSignals.length; i++) {
            signals.push(fileApi().findIndex((v) => (v == debug_api[1][i + (pageCounter() - 1) * 10])) < 0);
        }
        return signals;
    });
    ...
    return (
        <>
            ...
            <div class="mx-4">
                ...
                <select class="select select-sm w-full" onInput={(e) => {
                    ...
                    <div class="w-5/12">
                        ...
                        <ul class="menu menu-xs bg-base-100 rounded-box w-full">
                            <For each={filecheackSignals}>
                                {(signal, idx) => (
                                    <Show when={debug_api[1][idx() + (pageCounter() - 1) * 10] != undefined} fallback={<li><a class="h-6"></a></li>}>
                                        <li>
                                            <a onClick={() => {
                                                ...
                                                <input type="checkbox" checked={signal} class="checkbox checkbox-xs" disabled={!fileExistSignals()[idx()]} />{debug_api[1][idx() + (pageCounter() - 1) * 10].split("/").reverse()[0]}
                ...
                <div class="bg-base-100 rounded-box p-2">
                    ...
                    <Show when={apiCounter() > 0 && fileApi().length > 0} fallback={"No file data"}>
