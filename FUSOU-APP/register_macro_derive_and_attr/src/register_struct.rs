...
static STRUCT_NAMES: LazyLock<Mutex<HashSet<String, RandomState>>> = LazyLock::new(|| {
...
pub fn register_struct(attr: TokenStream, ast: &mut DeriveInput) -> Result<TokenStream, syn::Error> {
    ...
    match ast.data {
        syn::Data::Struct(_) => {
            ...
            }
            {
                let cfg: TestConfig = TestConfig{
                    struct_name: STRUCT_NAMES.lock().unwrap().clone().into_iter().collect::<HashSet<String, RandomState>>(),
                };
        ...
        _ => {
            return Err(syn::Error::new_spanned(&ast.ident, "#[register_struct] is only defined for structs, not for enums or unions, etc."));
        }
    }
