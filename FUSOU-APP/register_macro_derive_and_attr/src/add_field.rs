...
pub fn add_field(attr: TokenStream, ast: &mut DeriveInput) -> Result<TokenStream, syn::Error> {
    ...
    match ast.data {
            ...
            match &mut struct_data.fields {
                syn::Fields::Named(fields) => {
                    match args.struct_name {
                        Some(_) => { fields.named.push(syn::Field::parse_named.parse2(tokens_name.into()).unwrap()); },
                        None => ()
                    };
                    match args.extra {
                        Some(_) => { fields.named.push(syn::Field::parse_named.parse2(tokens_extra.into()).unwrap()); },
                        None => ()
                    };
                }
        ...
        _ => {
            return Err(syn::Error::new_spanned(&ast.ident, "#[add_field(...)] is only defined for structs, not for enums or unions, etc."));
        }
    }
