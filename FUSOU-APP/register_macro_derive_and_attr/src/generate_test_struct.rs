...
static STRUCT_ID: LazyLock<Mutex<HashSet<String, RandomState>>> = LazyLock::new(|| {
...
pub fn generate_test_struct(ast: &mut DeriveInput) -> Result<TokenStream, syn::Error> {
    ...
    match ast.data {
        syn::Data::Struct(ref mut struct_data) => {
            ...
            let repalce_bytes = re_bytes.replace_all(&repalce_span, "bytes: (_.._)").to_string();
            ...
            let struct_data_vec_u64 = ast_bytes.iter().enumerate().filter(|(i, _)| i % 8 == 0).map(
                |(i, _)| {
                    let mut bytes = [0u8; 8];
                    let i_8 = i*8;
