...
impl<T> Getter for Vec<T> where T: Getter {
...
impl<T> Getter for Option<T> where T: Getter {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        match self {
...
impl<T, U> Getter for HashMap<T, U> where T: Getter, U: Getter {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        for (k, v) in self {
            v.check_number(log_map, key.clone());
            k.check_number(log_map, (key.clone().unwrap().0, key.clone().unwrap().1, format!("key__{}", key.clone().unwrap().2)).into());
        }
    }
}
...
impl Getter for i8 {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        if let Some(key) = key {
            let bit_size = (0usize..7).rposition(|x| ((*self).abs() as u8 & (1u8 << x)) != 0).unwrap_or(0) as i64 + 1i64;
...
impl Getter for i16 {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        if let Some(key) = key {
            let bit_size = (0usize..15).rposition(|x| ((*self).abs() as u16 & (1u16 << x)) != 0).unwrap_or(0) as i64 + 1i64;
            if log_map.contains_key(&key) {
                let value = log_map.get_mut(&key).unwrap();
                value[0] = value[0].min(*self as i64);
                value[1] = value[1].max(*self as i64);
...
impl Getter for i32 {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        if let Some(key) = key {
            let bit_size = (0usize..31).rposition(|x| ((*self).abs() as u32 & (1u32 << x)) != 0).unwrap_or(0) as i64 + 1i64;
            if log_map.contains_key(&key) {
                let value = log_map.get_mut(&key).unwrap();
                value[0] = value[0].min(*self as i64);
                value[1] = value[1].max(*self as i64);
...
impl Getter for i64 {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        if let Some(key) = key {
            let bit_size = (0usize..64).rposition(|x| ((*self).abs() as u64 & (1u64 << x)) != 0).unwrap_or(0) as i64 + 1i64;
            if log_map.contains_key(&key) {
                let value = log_map.get_mut(&key).unwrap();
                value[0] = value[0].min(*self as i64);
                value[1] = value[1].max(*self as i64);
...
impl Getter for u8 {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        if let Some(key) = key {
            let bit_size = (0usize..8).rposition(|x| (*self & (1u8 << x)) != 0).unwrap_or(0) as i64;
            if log_map.contains_key(&key) {
                let value = log_map.get_mut(&key).unwrap();
                value[0] = value[0].min(*self as i64);
                value[1] = value[1].max(*self as i64);
...
impl Getter for u16 {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        if let Some(key) = key {
            let bit_size = (0usize..16).rposition(|x| (*self & (1u16 << x)) != 0).unwrap_or(0) as i64;
            if log_map.contains_key(&key) {
                let value = log_map.get_mut(&key).unwrap();
                value[0] = value[0].min(*self as i64);
                value[1] = value[1].max(*self as i64);
...
impl Getter for u32 {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        if let Some(key) = key {
            let bit_size = (0usize..32).rposition(|x| (*self & (1u32 << x)) != 0).unwrap_or(0) as i64;
            if log_map.contains_key(&key) {
                let value = log_map.get_mut(&key).unwrap();
                value[0] = value[0].min(*self as i64);
                value[1] = value[1].max(*self as i64);
...
impl Getter for u64 {
    fn check_number(&self, log_map: &mut LogMapNumberSize, key: Option<(String, String, String)>) {
        if let Some(key) = key {
            let bit_size = (0usize..64).rposition(|x| (*self & (1u64 << x)) != 0).unwrap_or(0) as i64;
            if log_map.contains_key(&key) {
                let value = log_map.get_mut(&key).unwrap();
                value[0] = value[0].min(*self as i64);
                value[1] = value[1].max(*self as i64);
...
fn write_log_check_number_size(log_path: String, log_map: &LogMapNumberSize) -> usize {
    ...
    writeln!(file, "check number size result [{}]", local).expect(&format!("\x1b[38;5;{}m cannot write.\x1b[m ", 8));
    file.write(format!("{:#?}", log_map).as_bytes()).expect(&format!("\x1b[38;5;{}m cannot write.\x1b[m ", 8));
...
pub fn simple_root_check_number_size<T>(target_path: String, pattren_str: String, log_path: String) where T: TraitForRoot + Getter {
...
pub fn custom_root_check_number_size<T>(file_list: impl Iterator<Item = PathBuf>, log_path: String) where T: TraitForRoot + Getter {
