...
async function runTest() {
  ...
  const clientSchema =
    '{"type":"record","name":"EnemyDeck","fields":[{"name":"env_uuid","type":{"type":"string","logicalType":"uuid"}},{"name":"uuid","type":{"type":"string","logicalType":"uuid"}},{"name":"ship_ids","type":["null",{"type":"string","logicalType":"uuid"}]}]}';

  // Server schema (without logicalType, as stored in schema_v0_5.json)
  const serverSchema =
  ...
  console.log("Server schema (without logicalType, as in schema_v0_5.json):");
  ...
  if (!result.matched) {
    ...
    console.log(
      "The canonical_form() strips logicalType on PARSE, not in stored string.",
    );
    console.log(
      "Server stores canonical (no logicalType), but compares with canonical of client (also no logicalType).",
    );
    console.log(
      "This should match... unless there's a difference in field order or structure.\n",
    );
  }
