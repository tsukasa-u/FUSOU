use crate::{
    certificate_authority::CertificateAuthority, Body, ClientStreamHook, HttpHandler,
    NoopClientStreamHook, NoopHandler, Proxy, WebSocketHandler,
};
...
impl<CA> ProxyBuilder<WantsClient<CA>> {
            ...
            .with_safe_default_protocol_versions()
        {
            ...
            Err(e) => {
                return ProxyBuilder(WantsHandlers {
                    ...
                    client_stream_hook: NoopClientStreamHook,
                });
            }
        };
    ...
    pub fn with_client<C>(
        ...
        WantsHandlers<CA, C, NoopHandler, NoopHandler, Pending<()>, NoopClientStreamHook>,
    >
    where
...
pub struct WantsHandlers<CA, C, H, W, F, K = NoopClientStreamHook> {
    al: AddrOrListener,
    ca: CA,
...
impl<CA, C, H, W, F, K> ProxyBuilder<WantsHandlers<CA, C, H, W, F, K>> {
    /// Set the HTTP handler.
    pub fn with_http_handler<H2: HttpHandler>(
        self,
        http_handler: H2,
    ) -> ProxyBuilder<WantsHandlers<CA, C, H2, W, F, K>> {
        ProxyBuilder(WantsHandlers {
            al: self.0.al,
            ca: self.0.ca,
            ...
            client_stream_hook: self.0.client_stream_hook,
        })
    }
    ...
    ) -> ProxyBuilder<WantsHandlers<CA, C, H, W2, F, K>> {
        ProxyBuilder(WantsHandlers {
            al: self.0.al,
            ca: self.0.ca,
            client: self.0.client,
    ...
    pub fn with_client_stream_hook<K2: ClientStreamHook>(
        self,
        client_stream_hook: K2,
    ) -> ProxyBuilder<WantsHandlers<CA, C, H, W, F, K2>> {
        ProxyBuilder(WantsHandlers {
            al: self.0.al,
            ca: self.0.ca,
            client: self.0.client,
    ...
    ) -> ProxyBuilder<WantsHandlers<CA, C, H, W, F2, K>> {
        ProxyBuilder(WantsHandlers {
            al: self.0.al,
            ca: self.0.ca,
            client: self.0.client,
    ...
    /// Build the proxy.
    pub fn build(self) -> Result<Proxy<C, CA, H, W, F, K>, crate::Error> {
        Ok(Proxy {
            al: self.0.al,
