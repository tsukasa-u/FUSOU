...
use crate::{
    builder::ProxyBuilder, certificate_authority::CertificateAuthority, Body, Error, HttpContext,
...
/// A client-facing MITM plaintext stream after TLS termination.
pub trait ClientStreamIo: AsyncRead + AsyncWrite + Unpin + Send {}

impl<T> ClientStreamIo for T where T: AsyncRead + AsyncWrite + Unpin + Send {}

pub type ClientStream = Box<dyn ClientStreamIo>;

/// Hook for wrapping the client-facing plaintext stream before Hyper parses it.
pub trait ClientStreamHook: Clone + Send + Sync + 'static {
...
pub struct NoopClientStreamHook;

impl ClientStreamHook for NoopClientStreamHook {
    fn wrap(&self, _context: &HttpContext, stream: ClientStream) -> ClientStream {
        stream
    }

    fn finish(&self, _context: HttpContext) -> Pin<Box<dyn Future<Output = ()> + Send>> {
        Box::pin(async {})
    }
}
...
/// # fn main() {}
/// ```
pub struct Proxy<C, CA, H, W, F, K = NoopClientStreamHook> {
    al: AddrOrListener,
    ca: Arc<CA>,
    ...
    graceful_shutdown: F,
    client_stream_hook: K,
}

impl Proxy<(), (), (), (), (), NoopClientStreamHook> {
...
impl<C, CA, H, W, F, K> Proxy<C, CA, H, W, F, K>
where
        ...
        loop {
            tokio::select! {
                res = listener.accept() => {
                    ...
                    let client_stream_hook = self.client_stream_hook.clone();
                        ...
                        let conn = server.serve_connection_with_upgrades(
                            ...
                            service_fn(|req| {
                                InternalProxy {
                                    ...
                                    client_stream_hook: client_stream_hook.clone(),
                                    client_addr,
                                    connection_id,
                                }
                                .proxy(req)
