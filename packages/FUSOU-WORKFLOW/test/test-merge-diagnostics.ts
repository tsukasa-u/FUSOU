...
if (process.argv.length < 3) {
  console.error(
    "Usage: npx tsx test-merge-diagnostics.ts <path-to-parquet-file>",
  );
  process.exit(1);
}
...
try {
  ...
  console.log(
    `Header magic (bytes 0-3): '${headerMagic}' ${headerMagic === "PAR1" ? "✓" : "✗ INVALID"}`,
  );

  // Check footer
  const footerMagic = String.fromCharCode(
    buf[buf.length - 4],
    buf[buf.length - 3],
    buf[buf.length - 2],
    buf[buf.length - 1],
  ...
  console.log(
    `Footer magic (bytes -4 to -1): '${footerMagic}' ${footerMagic === "PAR1" ? "✓" : "✗ INVALID"}`,
  );
  ...
  try {
    ...
    const footerSnippet = buf.slice(
      footerStart,
      Math.min(footerStart + 64, buf.length - 8),
    );
    console.log(
      Array.from(footerSnippet)
  ...
  for (let i = 0; i < rowGroups.length; i++) {
    ...
    console.log(
      `  totalByteSize: ${hasSize ? rg.totalByteSize : "(undefined)"}`,
    );
    ...
    if (rgEnd > Number.MAX_SAFE_INTEGER || rgEnd < rg.offset) {
      console.log(
        `  ✗ INVALID: integer overflow in end position (offset=${rg.offset}, size=${rg.totalByteSize})`,
      );
      invalidCount++;
      console.log();
      continue;
    ...
    if (rgStart < 0) {
      ...
      console.log(
        `  ✗ INVALID: range [${rgStart}, ${rgEnd}) exceeds data region (0, ${footerStart})`,
      );
      invalidCount++;
    } else {
