...
async function fetchHotData(env: Env, params: QueryParams): Promise<any[]> {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      console.error(
        `[Reader] Failed to parse Hot data row ${row.id}:`,
...
async function parseSchemaFingerprintFromHeader(
  header: Uint8Array,
): Promise<{ fingerprint: string | null; namespace: string | null }> {
  try {
...
async function validateHeaderTableVersion(
  ...
  if (allowedList.length > 0 && !allowedList.includes(fingerprint)) {
    throw new Error(
      `Schema fingerprint mismatch for table_version=${tableVersion}/${tableName}`,
    );
  }
}
...
async function fetchColdData(
  ...
  for (const [filePath, blocks] of fileGroups.entries()) {
    ...
    await validateHeaderTableVersion(
      header,
      fileTableVersion,
      allowedFingerprints,
      tableName,
    ...
    const blockPromises = blocks.map((block) => {
      if (
        block.compression_codec &&
        headerCodec &&
        block.compression_codec !== headerCodec
      ) {
        throw new Error(
          `Compression codec mismatch for ${filePath}: header=${headerCodec}, index=${block.compression_codec}`,
        );
      }
      return fetchColdBlock(
        r2,
        filePath,
        block.start_byte,
        block.length,
...
export async function handleRead(
  ...
  try {
    ...
    if ((params.format ?? "json") === "ocf") {
      ...
      if (uniqueVersions.size > 1) {
        ...
        return new Response(
          JSON.stringify({
            ...
            details:
              "OCF format requires single table version. Please specify table_version parameter.",
          }),
          {
            status: 400,
      ...
      const blockPromises = blocks.map((b) =>
        fetchColdBlock(
    ...
    return new Response(
      JSON.stringify({
        ...
        table_name: params.table_name,
        table_version:
          detectedVersionsList.length === 1
            ? detectedVersionsList[0]
            : undefined,
