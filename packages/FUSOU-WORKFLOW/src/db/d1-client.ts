...
export async function cleanupBuffer(
  ...
  const result = await db
    .prepare("DELETE FROM buffer_logs WHERE id <= ?")
    .bind(maxId)
    .run();
 ...
 */
export async function getBufferedCount(db: D1Database): Promise<number> {
  const result = await db
    .prepare("SELECT COUNT(*) as count FROM buffer_logs")
    .first<{ count: number }>();
