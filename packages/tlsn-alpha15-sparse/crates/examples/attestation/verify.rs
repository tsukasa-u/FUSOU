...
async fn verify_presentation(example_type: &ExampleType) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sent = String::from_utf8_lossy(&partial_transcript.materialize_sent());
    let recv = String::from_utf8_lossy(&partial_transcript.materialize_received());

    println!("-------------------------------------------------------------------");
    println!(
        "Successfully verified that the data below came from a session with {server_name} at {time}.",
    );
    println!("Note that undisclosed bytes are materialized as zero bytes.\n");
    println!("Data sent:\n");
    println!("{sent}\n");
