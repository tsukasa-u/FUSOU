...
impl TranscriptProof {
    ...
    pub fn verify_with_provider<'a>(
        ...
        } in self.hash_secrets
        {
            ...
            if !self.transcript.contains(direction, &idx) {
                return Err(TranscriptProofError::new(
                    ErrorKind::Hash,
            ...
            buffer.clear();
            self.transcript
                .copy_authenticated_to(direction, &idx, &mut buffer)
                .map_err(|error| TranscriptProofError::new(ErrorKind::Hash, error))?;
...
mod tests {
    ...
    fn test_reveal_with_hash_commitment(#[case] alg: HashAlgId) {
        ...
        assert_eq!(
            partial_transcript.materialize_sent()[0..10],
