...
use tlsn_core::transcript::{Direction, PartialTranscript, Record};
...
pub(crate) fn verify_plaintext<'a>(
    ...
    let transcript_len = match direction {
        Direction::Sent => transcript.len_sent(),
        Direction::Received => transcript.len_received(),
    };
    let is_reveal_all = reveal == (0..transcript_len);
    ...
    let plaintext_proof = if is_reveal_all {
        let mut plaintext = Vec::with_capacity(transcript_len);
        transcript
            .copy_authenticated_to(
                direction,
                &RangeSet::from(0..transcript_len),
                &mut plaintext,
            )
            .map_err(PlaintextAuthError::sparse)?;
            ...
            .iter()
        {
            ...
            transcript
                .copy_authenticated_to(direction, &RangeSet::from(range.clone()), &mut plaintext)
                .map_err(PlaintextAuthError::sparse)?;
            vm.assign(*slice, plaintext)
                .map_err(PlaintextAuthError::vm)?;
        }
...
impl PlaintextAuthError {
    ...
    fn sparse(err: tlsn_core::transcript::PartialTranscriptReadError) -> Self {
        Self(ErrorRepr::Sparse(err.to_string()))
    }
}
...
enum ErrorRepr {
    ...
    MissingDecoding,
    #[error("sparse transcript read failed: {0}")]
    Sparse(String),
