...
// POST /asset-sync/upload
app.post('/asset-sync/upload', async (c) => {
  const bucket = c.env.ASSET_SYNC_BUCKET;
  const db = c.env.ASSET_INDEX_DB;
  const signingSecret = c.env.ASSET_UPLOAD_SIGNING_SECRET || import.meta.env.ASSET_UPLOAD_SIGNING_SECRET;
  ...
  if (!url.searchParams.has('token')) {
    return await handleSignedUploadRequest(c, request, bucket, db, allowedExtensions, signingSecret, url);
  }

  return await handleSignedUploadExecution(c, request, bucket, db, allowedExtensions, signingSecret, url);
});

// GET /asset-sync/keys
app.get('/asset-sync/keys', async (c) => {
  const db = c.env.ASSET_INDEX_DB;
  ...
  try {
    ...
    while (true) {
      ...
      const batch = (res?.results || [])
        .map((r) => (typeof (r as any).key === 'string' ? (r as any).key : undefined))
...
// GET /asset-sync/mime
app.get('/asset-sync/mime', async (c) => {
...
async function handleSignedUploadRequest(
  ...
  const relativePath = sanitizeKey(typeof bodyObj.relative_path === 'string' ? bodyObj.relative_path : null);
  if (!relativePath) {
    return c.json({ error: 'Invalid relative_path' }, 400);
  }

  const declaredSize = parseSize(typeof bodyObj.file_size === 'string' ? bodyObj.file_size : undefined);
  if (!declaredSize || declaredSize <= 0 || declaredSize > MAX_UPLOAD_BYTES) {
    return c.json({ error: 'Invalid file size' }, 400);
  }

  const fileName = sanitizeFileName(typeof bodyObj.file_name === 'string' ? bodyObj.file_name : null);
  ...
  return c.json({
    ...
    fields: { key, relative_path: relativePath, declared_size: declaredSize, file_name: fileName },
  });
}
...
async function handleSignedUploadExecution(
  ...
  try {
    ...
    await stmt
      .bind(key, storedSize, uploadedAt, 'application/octet-stream', supabaseUser.id, null, JSON.stringify({ synced_at: uploadedAt }))
      .run();
