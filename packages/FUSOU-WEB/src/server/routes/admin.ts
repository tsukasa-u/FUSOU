...
adminApp.get("/fix-mime-types", async (c) => {
  ...
  const limit = Math.max(
    1,
    Math.min(1000, parseInt(c.req.query("limit") || "100", 10) || 100),
  );
...
adminApp.get("/backfill-asset-index", async (c) => {
  ...
  try {
    ...
    let bumpedRevision: number | null = null;
    if (!dryRun && updated > 0) {
      bumpedRevision = await bumpAssetIndexRevision(db);
    }
