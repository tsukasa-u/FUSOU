...
app.post('/google/refresh_token', async (c) => {
  ...
  try {
    const supabase = createClient(
      ...
      (c.env.PUBLIC_SUPABASE_PUBLISHABLE_KEY || import.meta.env.PUBLIC_SUPABASE_PUBLISHABLE_KEY)!,
      {
    ...
    return c.json({ accessToken: data.access_token, expiresIn: data.expires_in });
  } catch (error) {
...
app.post('/local_auth/signin', async (c) => {
  return c.json({ message: 'Use Astro native local_auth/signin endpoint' }, 400);
});

app.get('/local_auth/callback', async (c) => {
  return c.json({ message: 'Use Astro native local_auth/callback endpoint' }, 400);
});
