...
export default function SidePage() {
  ...
  const [show_rename_page_dialog, set_show_rename_page_dialog] =
    createSignal(false);
  ...
  const check_name = (): string => {
    ...
    if (
      $pageData().some((v) => v.name == page_title) ||
      page_title == "Untitled"
    ) {
      let index = 1;
  ...
  return (
    <>
      {$pageData()
        .map((v) => [v.id, v] as [string, PageInfo])
        .map(([unique_id, sidePageItem]) => (
          <li>
            <div
              class={
                side_page_selected() == unique_id
                  ? "join bg-orange-500 p-0"
                  : "join p-0"
              }
            >
              <div
                class="truncate join-item py-2 px-3"
                onClick={() => {
                  set_side_page_selected(unique_id);
                }}
              >
                {sidePageItem.name}
              </div>
              ...
              <div class="dropdown dropdown-end join-item">
                <div
                  tabindex="0"
                  role="button"
                  class="h-9 w-10 flex justify-center items-center"
                >
                  <svg
                ...
                >
                  <li>
                    <a onClick={() => duplicate(unique_id)}>Duplicate</a>
                  </li>
                  <li>
                    <a
                      onClick={() => {
      ...
      <li>
        <a
          onClick={() => show_modal("add_page_modal", set_show_add_page_dialog)}
        >
      ...
      <Show when={show_add_page_dialog()}>
        <dialog id="add_page_modal" class="modal">
          <div class="modal-box">
            <form method="dialog">
              <button
                ...
                onClick={() => hide_modal(set_show_add_page_dialog)}
              >
                x
              </button>
            </form>
            ...
            <fieldset class="fieldset">
              <input
                ...
                onInput={(e) => {
                  let check_result = $pageData().some(
          ...
          <form method="dialog" class="modal-backdrop">
            <button onClick={() => hide_modal(set_show_add_page_dialog)}>
              close
            </button>
          </form>
        </dialog>
        ...
        <dialog id="rename_dialog" class="modal">
          <div class="modal-box">
            <form method="dialog">
              <button
                class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2"
                onClick={() => hide_modal(set_show_rename_page_dialog)}
              >
                x
              </button>
            </form>
            ...
            <fieldset class="fieldset">
              <input
                ...
                placeholder="Untitled"
                class="input w-full focus:outline-0"
                value={getPageData(rename_index()).name}
                onInput={(e) => {
                  let check_result = $pageData().some(
                    (v) => v.name == e.target.value
                  );
          ...
          <form method="dialog" class="modal-backdrop">
            <button onClick={() => hide_modal(set_show_rename_page_dialog)}>
              close
            </button>
          </form>
        </dialog>
