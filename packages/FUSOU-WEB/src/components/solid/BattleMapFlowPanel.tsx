...
export default function BattleMapFlowPanel() {
  ...
  const [showOfficialMapAssets, setShowOfficialMapAssets] = createSignal(false);
  ...
  async function ensureMapMetadata(mapKey: string): Promise<void> {
    ...
    try {
      try {
        ...
        if (imageMetaResponse.ok) {
          ...
          if (parsed) {
            ...
            addMetadataWarning(`${mapKey} の画像メタデータを解析できません。`);
          }
        } else {
          addMetadataWarning(`${mapKey} の画像メタデータ取得に失敗しました。`);
        }
      } catch {
        addMetadataWarning(`${mapKey} の画像メタデータ取得に失敗しました。`);
      }
      ...
      if (!response.ok) {
        addMetadataWarning(`${mapKey} の座標メタデータ取得に失敗しました。`);
        return;
      }
      ...
      if (asset.labelsUrl && !mapLabelsByKey()[mapKey]) {
        try {
          ...
          if (labelsResponse.ok) {
            ...
            addMetadataWarning(`${mapKey} のセルラベル取得に失敗しました。番号ラベルで表示します。`);
          }
        } catch {
          addMetadataWarning(`${mapKey} のセルラベル取得に失敗しました。番号ラベルで表示します。`);
        }
  ...
  onMount(() => {
    ...
    if (saved) {
      try {
        ...
        setShowOfficialMapAssets(!!parsed.showOfficialMapAssets);
  ...
  return (
    <>
      ...
      <div class="card bg-base-100 shadow-sm mb-6">
        <div class="card-body">
          ...
          <div class="text-xs text-base-content/60 mb-2">
            港からどの順番で進んだかを矢印で表示します。各経路のバッジはその遷移を通過した回数です。緑の点線は line 情報から推定した接続候補です。セルをクリックすると、そのマスに到達した進路と戦闘情報へ切り替わります。
          </div>
          <Show
            when={selectedRouteOverlay()}
            fallback={
              <div class="flex items-center justify-center h-64 text-base-content/40">
                {loading() ? "読込中..." : "海域ルートの座標データを読込中です"}
              </div>
            }
          >
            {(overlay) => (
              <div class="space-y-4">
                ...
                <div class="grid gap-3 md:grid-cols-3">
                  <div class="rounded-box bg-base-200 p-3 text-sm">
                    ...
                    <div class="text-xs text-base-content/70">緑の点線: マップ座標情報から推定した接続候補</div>
      ...
      <div class="card bg-base-100 shadow-sm mt-6">
        <div class="card-body">
          <h3 class="card-title text-lg">進軍ルート可視化（ソーティー別）</h3>
