  ...
  async queue(batch: MessageBatch<any>, env: Bindings) {
    for (const msg of batch.messages) {
      ...
      try {
        ...
        console.log("[Compaction Queue] Compaction finished", { datasetId, table, result });
        msg.ack();
      } catch (err) {
        console.error("[Compaction Queue] Failed", { datasetId, table, error: String(err) });
        ...
        try {
          ...
          console.error("[Compaction Queue] DLQ send failed", { error: String(dlqErr) });
        }
