...
export function EquimentComponent(props: EquipmentProps) {
  const [_mst_slot_items] = useMstSlotItems();
  const [_slot_items] = useSlotItems();

  const slot_item = createMemo(() => {
    return _slot_items.slot_items[props.slot_id];
  });

  const mst_slot_item = createMemo(() => {
    return _mst_slot_items.mst_slot_items[
  ...
  const proficiency = createMemo<JSX.Element>(() => {
    let proficiency: JSX.Element = <div class="h-2 w-2" />;
    if (slot_item()?.alv == 1)
      proficiency = <IconPlaneProficiency1 class="h-2 w-2" />;
    else if (slot_item()?.alv == 2)
      proficiency = <IconPlaneProficiency2 class="h-2 w-2" />;
    else if (slot_item()?.alv == 3)
      proficiency = <IconPlaneProficiency3 class="h-2 w-2" />;
    else if (slot_item()?.alv == 4)
      proficiency = <IconPlaneProficiency4 class="h-2 w-2" />;
    else if (slot_item()?.alv == 5)
      proficiency = <IconPlaneProficiency5 class="h-2 w-2" />;
    else if (slot_item()?.alv == 6)
      proficiency = <IconPlaneProficiency6 class="h-2 w-2" />;
    else if (slot_item()?.alv == 7)
      proficiency = <IconPlaneProficiency7 class="h-2 w-2" />;
    return proficiency;
  });
  ...
  return (
    <>
      ...
      >
        ...
        {!(props.ex_flag ?? false) ? (
          <div class="flex-none pl-0.5 -mt-[3px]">
            <div class="grid h-2.5 w-4 place-content-center text-xs text-accent">
              {proficiency()}
            </div>
            <div class="grid h-2.5 w-4 place-content-center text-xs">
              {show_onslot() ? props.onslot : ""}
            </div>
          </div>
        ) : (
          <div class="flex-none pl-px">
            <div class="w-4" />
          </div>
        )}
        {(props.name_flag ?? false) ? (
          <div class="pl-3 truncate">{mst_slot_item()?.name ?? "Unknown"}</div>
        ) : (
          <></>
        )}
      </div>
      ...
      <Show when={show_dialog()}>
        <dialog id={"deck_equipment_modal_" + props.slot_id} class="modal">
          <div class="modal-box bg-base-100 modal-box-width">
            ...
            <div
              class="flex justify-start tooltip tooltip-bottom"
              data-tip={display_tooltip()}
            >
              {/* <div class="flex justify-start"> */}
              <h3 class="font-bold text-base pl-3 truncate">
                {mst_slot_item()?.name ?? "Unknown"}
              </h3>
              <div class="place-self-end pb pl-4 text-sm text-accent">
                {signed_number(slot_item()?.level ?? 0)}
              </div>
            </div>
            <div class="pt-2">
