...
<body>
  ...
  <div class="container">
    ...
    <div class="sub-panel">
      ...
      <div class="section" id="clustering-section">
        ...
        <div class="clustering-info" id="clustering-info" style="font-size:10px;color:#9ca3af;margin-top:4px;">No clustering data</div>
      </div>
  ...
  <script>
    ...
    function drawDatasetPlot() {
      ...
      // 全次元を表示: 各特徴量 vs ターゲットのサブプロット
      const traces = [];
      for (let i = 0; i < Math.min(numFeatures, 5); i++) {
      ...
      // レイアウト: 水平に並べる
      const layout = {
        grid: { rows: 1, columns: Math.min(numFeatures, 5), pattern: 'independent' },
    ...
    function drawClusteringPlot() {
      console.log('drawClusteringPlot called', clusterData);
      if (!clusterData) {
        console.log('No clusterData');
        return;
      }
      if (!clusterData.features || !clusterData.clusters) {
        console.log('Missing features or clusters', clusterData);
        return;
      }
      ...
      console.log('Features:', features.length, 'Clusters:', clusters.length);
      ...
      const traces = uniqueClusters.map(cl => {
        ...
        const x = idx.map(i => features[i][0]);
        const y = idx.map(i => numFeatures > 1 ? features[i][1] : features[i][0]);
        return {
          x: x,
          y: y,
          mode: 'markers',
      ...
      console.log('Drawing', traces.length, 'traces');
      Plotly.newPlot('clustering-plot', traces, {
        margin: { t: 20, l: 40, r: 10, b: 40 },
        xaxis: { title: featureNames[0] || 'Feature 0', color: '#9ca3af', gridcolor: '#1e293b' },
        yaxis: { title: numFeatures > 1 ? (featureNames[1] || 'Feature 1') : (featureNames[0] || 'Feature 0'), color: '#9ca3af', gridcolor: '#1e293b' },
        paper_bgcolor: '#111827',
        plot_bgcolor: '#111827',
        font: { color: '#e5e7eb' },
      }, {displayModeBar: false});
      addLog('Clustering plot updated: ' + uniqueClusters.length + ' clusters', 'info');
    }
