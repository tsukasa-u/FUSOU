...
assert.equal(EVIDENCE_WORKER_SECRET_CONTRACT.main.includes("TLSN_DIRECT_CALLBACK_SECRET"), false);
...
assert.equal(EVIDENCE_WORKER_SECRET_CONTRACT.main.includes("TLSN_TEST_DIRECT_INVOCATION_TIMEOUT_MS"), true);
assert.equal(EVIDENCE_WORKER_SECRET_CONTRACT.verifier.includes("TLSN_TEST_DIRECT_VERIFIER_MODE"), true);
...
const testDummyEnvironment = Object.fromEntries(
  [...new Set(Object.values(TEST_WORKER_SECRET_CONTRACT).flat())]
    .map((name) => [name, `dummy-${name}`]),
);
for (const worker of ["main", "verifier"]) {
  const bundle = workerSecretBundleForTest(worker, testDummyEnvironment);
  assert.deepEqual(Object.keys(bundle), TEST_WORKER_SECRET_CONTRACT[worker]);
  assert.equal(bundle.TLSN_TRUST_ROOT_CERTIFICATE_DER, undefined, `${worker} received trust root as a secret`);
}
assert.equal(TEST_WORKER_SECRET_CONTRACT.main.includes("TLSN_DIRECT_CALLBACK_SECRET"), false);
assert.ok(TEST_WORKER_SECRET_CONTRACT.verifier.includes("TLSN_DIRECT_CALLBACK_SECRET"));
assert.ok(TEST_WORKER_SECRET_CONTRACT.main.includes("TLSN_TRIGGER_SECRET_KEY"));
assert.ok(TEST_WORKER_SECRET_CONTRACT.main.includes("TLSN_QUEUE_CALLBACK_SECRET"));
assert.equal(TEST_WORKER_SECRET_CONTRACT.verifier.includes("TLSN_TEST_AUTH_USERS"), false);
assert.equal(TEST_WORKER_SECRET_CONTRACT.verifier.includes("TLSN_TEST_BINDING_VALUE"), false);
assert.equal(TEST_WORKER_SECRET_CONTRACT.verifier.includes("TLSN_TRIGGER_SECRET_KEY"), false);
assert.equal(TEST_WORKER_SECRET_CONTRACT.verifier.includes("TLSN_QUEUE_CALLBACK_SECRET"), false);
assert.equal(TEST_WORKER_PUBLIC_INPUTS.verifier.includes("TLSN_TEST_DIRECT_SYNCHRONOUS_CANDIDATE"), false);
assert.ok(TEST_WORKER_SECRET_CONTRACT.verifier.includes("TLSN_TEST_DIRECT_VERIFIER_MODE"));
