...
try {
  ...
  await writeFile(join(profileDirectory, "notary-export.json"), `${JSON.stringify({
    ...
    key_id: "notary-production-2026",
    status: "ACTIVE",
    signature_algorithm: "secp256k1",
    verifying_key_base64url: fixture.notary_key_base64,
    sec1_public_key_base64url: notaryPublicKeyBytes.subarray(9).toString("base64url"),
  })}\n`);
  ...
  assert.equal(complete.generatedEnv.TLSN_CANDIDATE_NOTARY_ENDPOINT, "notary.canary.example.net:7047");
  ...
  assert.equal(complete.manifest.notary.source, "fusou_notary_public_key_export");
  assert.equal(complete.manifest.notary.endpoint, "notary.canary.example.net:7047");
  assert.equal(complete.manifest.notary.provisioning_record.owner, "FUSOU");
