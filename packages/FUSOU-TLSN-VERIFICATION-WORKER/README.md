...
`pnpm run capture:production-evidence` is a standalone live capture harness. It requires a real production Worker, FUSOU-WEB and Supabase origins, a non-anonymous access token, a registered device private key, a Presentation fixture, the production result signing registry, and the production Notary registry (`TLSN_PRODUCTION_NOTARY_REGISTRY` or `TLSN_CANDIDATE_NOTARY_REGISTRY`). Fixture member IDs, request/response bytes, server identities, and provenance declarations are diagnostic-only; the alpha.15 verifier derives the server identity, transcript digests/ranges, HTTP profile, and member ID directly from the cryptographically verified Presentation. The harness issues a fresh session, submits the device possession proof and Presentation, independently verifies the signed Result, and verifies one-shot replay rejection. It writes protected Presentation, trust-registry, semantic-verification, Result, and session artifacts plus a signed manifest when the separate evidence signer is available.
