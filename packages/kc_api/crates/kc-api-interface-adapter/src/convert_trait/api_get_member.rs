...
impl TraitForConvert for ship2::Res {
    ...
    fn convert(&self) -> Option<Vec<EmitData>> {
        ...
        let entries = self
            ...
            .filter_map(|s| {
                ...
                Some(ShipGrowthEntry {
                    ...
                    sp_effect_items_json: s.api_sp_effect_items.as_ref().and_then(|items| serde_json::to_string(items).ok()),
                    kaihi_observed,
                    taisen_observed,
                    sakuteki_observed,
                    lucky_observed,
        ...
        let mut emit_data = vec![EmitData::Set(Set::ShipGrowthSnapshot(ShipGrowthSnapshot {
            entries,
        }))];
...
impl TraitForConvert for ship3::Res {
    ...
    fn convert(&self) -> Option<Vec<EmitData>> {
        ...
        if !soku_speed_entries_3.is_empty() {
            emit_data.push(EmitData::Set(Set::SokuSpeedObservedSnapshot(
                SokuSpeedObservedSnapshot {
