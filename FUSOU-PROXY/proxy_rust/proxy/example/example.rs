...
async fn main () {
    ...
    let proxy_server_addr = proxy_server::serve_proxy(proxy_address.to_string(), 0, rx_proxy, tx_proxy_log).await.unwrap();
    let pac_server_addr = pac_server::serve_pac_file("proxy.pac".to_string(), 0, rx_pac).await.unwrap();
