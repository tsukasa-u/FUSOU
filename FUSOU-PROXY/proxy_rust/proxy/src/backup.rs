...
async fn log_response<F>(mut response: Response<Body>, f: F) -> Result<Response<Body>, Rejection> where  F: Fn(&Vec<u8>)+ Send + Sync {
...
// async fn log_request(req: warp::http::Request<Body>) -> Result<warp::http::Request<Body>, Rejection> {
//     println!("{:?}", req);
//     Ok(req)
// }
...
async fn main() {
    ...
    let (tx_proxy, rx_proxy) = oneshot::channel::<bool>();
    let (tx_pac, rx_pac) = oneshot::channel::<bool>();

    pac_server::serve_pac_file("proxy.pac".to_string(), 8000, rx_pac).await.unwrap();

    let test_closure = |res: &Vec<u8>|{
        println!("{:?}", res);
    };
    ...
    .and(
        ...
        .and_then(move |response: Response<Body>| async move {
            log_response(response, test_closure).await
        })
    );
    // spawn proxy server
    let (addr, server_proxy) = warp::serve(app).bind_with_graceful_shutdown(([127, 0, 0, 1], 3030), async move {
        tokio::select! {
            _ = rx_proxy => {},
