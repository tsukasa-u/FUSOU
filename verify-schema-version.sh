...
# Used to validate the table_version integration
...
if grep -r "use kc_api::database::DATABASE_TABLE_VERSION" "$WORKSPACE_ROOT/FUSOU-APP/src-tauri/src/" 2>/dev/null; then
    echo "✓ Found kc_api::database::DATABASE_TABLE_VERSION imports"
else
    echo "❌ FAIL: Expected kc_api::database::DATABASE_TABLE_VERSION not found"
    exit 1
fi
...
if grep -q 'kc_api = .*features.*schema_v0_4' "$FUSOU_APP_CARGO"; then
    echo "✓ kc_api configured with schema_v0_4 feature"
else
    echo "❌ FAIL: kc_api schema_v0_4 feature not configured"
    exit 1
fi
...
if grep -q 'schema_v0_4 = \["kc-api-database/schema_v0_4"\]' "$KC_API_CARGO"; then
    echo "✓ kc_api has schema_v0_4 feature definition"
else
    echo "❌ FAIL: schema_v0_4 feature definition not found"
    exit 1
fi

if grep -q 'schema_v0_5 = \["kc-api-database/schema_v0_5"\]' "$KC_API_CARGO"; then
    echo "✓ kc_api has schema_v0_5 feature definition"
else
    echo "❌ FAIL: schema_v0_5 feature definition not found"
    exit 1
fi
...
if grep -q 'schema_v0_4 = \[\]' "$KCA_DB_CARGO"; then
    echo "✓ kc-api-database has schema_v0_4 feature"
else
    echo "❌ FAIL: schema_v0_4 feature not found in kc-api-database"
    exit 1
fi

if grep -q 'schema_v0_5 = \[\]' "$KCA_DB_CARGO"; then
    echo "✓ kc-api-database has schema_v0_5 feature"
else
    echo "❌ FAIL: schema_v0_5 feature not found in kc-api-database"
    exit 1
fi
...
TABLE_VERSION_RS="$WORKSPACE_ROOT/kc_api/crates/kc-api-database/src/schema_version.rs"
if grep -q 'pub const DATABASE_TABLE_VERSION: &str' "$TABLE_VERSION_RS"; then
    echo "✓ DATABASE_TABLE_VERSION constant is defined"
else
    echo "❌ FAIL: DATABASE_TABLE_VERSION constant not found"
    exit 1
fi

if grep -q '#\[cfg(feature = "schema_v0_4")\]' "$TABLE_VERSION_RS"; then
    echo "✓ schema_v0_4 conditional compilation is present"
else
    echo "❌ FAIL: schema_v0_4 feature guard not found"
    exit 1
fi

if grep -q '#\[cfg(feature = "schema_v0_5")\]' "$TABLE_VERSION_RS"; then
    echo "✓ schema_v0_5 conditional compilation is present"
else
    echo "❌ FAIL: schema_v0_5 feature guard not found"
    exit 1
fi
...
if cargo check --message-format=short 2>&1 | grep -q "error\["; then
    ...
    echo "✓ FUSOU-APP builds successfully with schema_v0_4 feature"
fi
...
if cargo check --features "schema_v0_4,schema_v0_5" 2>&1 | grep -q "error:"; then
    echo "✓ Feature enforcement works: cannot use both schema_v0_4 and schema_v0_5"
...
echo "- schema_v0_4 and schema_v0_5 features are properly configured"
