...
- 実装済み:
  ...
  - `GET /api/attestation/config` 実装（署名付き設定配布）
  - `config_sync.rs` 実装（署名検証・version/expiry/issued_at 検証・rollback 検出・サイズ上限・キャッシュ署名検証）
  - `attestation/mod.rs` で config 優先化、ローカル CA フォールバック停止
  - ローカル CA 自動生成関数群の削除
  - Windows TPM: restricted AK 化 + 既存ハンドル属性検証追加
  - APP 側 attestation 設定の一元化（`configs.toml` の `app.auth.attestation_config_endpoint` + dotenvx 注入 `APP_ATTESTATION_CONFIG_SIGNING_PUBLIC_KEY`）
  - Trusted root の解決経路を `Supabase table (canonical) -> KV cache -> env fallback` へ拡張
- 未実装/暫定:
...
- 本書 v4.2 の差分項目のうち、以下は反映済み
  - ローカルCA自動生成フォールバックの撤去
  - `attestation/config` 同期経路の実装
  - Windows TPM の restricted AK 化と既存ハンドル属性検証追加
- 継続課題
  - macOS Secure Enclave 実装（現状 stub）
  - サーバー検証仕様（X.509/OCSP/CRL/URL制限）の文書整合を継続
...
| **予算方針** | 基本的にオープンソースまたはOS標準機能で構成。**新規プロダクトDBは追加しない（既存 Supabase 内でテーブル拡張）** | ― |
...
### 1.6 サーバー側シークレット入力の用途定義（Attestation）

> 2026-07-03 更新: trusted root の canonical source は Supabase `public.attestation_trusted_roots`。
> `INTEGRITY_*` は break-glass fallback（非常時のみ）へ役割変更。

| 設定キー/シークレット名 | 役割 | 形式 | 必須度 | 備考 |
| --- | --- | --- | --- | --- |
| `public.attestation_trusted_roots` (Supabase table) | TPM/SE trust anchor allowlist の canonical source | 行指向（`platform`,`root_sha256`,`status`,`valid_from`,`valid_to`） | 本番必須 | Worker はまずここを参照し、KV に短TTLキャッシュ |
| `INTEGRITY_TPM_AK_TRUSTED_ROOT_SHA256` | TPM trusted roots の fallback | JSON 配列 / CSV / 空白区切りの 64 桁 hex | 非常時のみ | Supabase/KV が利用不能なときの break-glass |
| `INTEGRITY_SECURE_ENCLAVE_TRUSTED_ROOT_SHA256` | Secure Enclave trusted roots の fallback | JSON 配列 / CSV / 空白区切りの 64 桁 hex | 非常時のみ | Supabase/KV が利用不能なときの break-glass |
| `app.auth.attestation_config_endpoint` | FUSOU-APP が署名付き attestation config を取得する URL | `https://` URL | クライアント側で必須 | `configs.toml` で一元管理。散在した個別設定を禁止 |
| `APP_ATTESTATION_CONFIG_SIGNING_PUBLIC_KEY` | FUSOU-APP が署名検証に使う公開鍵 | base64 32-byte Ed25519 公開鍵 | クライアント側で必須 | dotenvx で注入する。未設定時は fail-closed |

補足:

- 「アップロード可否」と「`hw_verified` 取得可否」は別である
- trusted root 未設定でもアップロード自体は可能（`sw_verified` / `unverified` / `suspicious` で運用）
- 読み取りコスト最適化のため、trusted roots は Worker で `DATA_LOADER_CACHE_KV` に 300 秒キャッシュする

### 1.6.1 Supabase trusted root テーブル仕様（固定）

テーブル: `public.attestation_trusted_roots`

- `platform`: `tpm` / `secure_enclave`
- `root_sha256`: lower-case 64 hex
- `status`: `active` / `staged` / `retired` / `blocked`
- `manufacturer`: 管理上の識別ラベル
- `source`: 取得元（URL/文書参照）
- `valid_from`, `valid_to`: 時間窓（null 可）

読み取り条件（Worker）:

1. `status = active`
1. 現在時刻が `valid_from..valid_to` 範囲内（null は無制限）
1. 取得結果を KV (`attestation-trusted-roots:v1:active`) に 300 秒保持
1. Supabase/KV 障害時のみ `INTEGRITY_*` fallback を使用

### 1.7 設定ファイルとクライアント同期の実装方式（追加調査不要の具体版）
...
クライアント同期は署名付き設定ドキュメントで行い、クライアント側の URL / 検証鍵参照先は `configs.toml` で一元管理する。

1. サーバーは `/api/attestation/config` を配信
1. サーバーは JSON 本文に Ed25519 署名を付与し、`X-FUSOU-Config-Signature` ヘッダーで返す
1. クライアントは `APP_ATTESTATION_CONFIG_SIGNING_PUBLIC_KEY` を読み込み、署名検証に成功した場合のみローカル反映
...
1. `APP_ATTESTATION_CONFIG_SIGNING_PUBLIC_KEY` で `X-FUSOU-Config-Signature` を検証
...
1. 新公開鍵を配布し、`APP_ATTESTATION_CONFIG_SIGNING_PUBLIC_KEY` を更新した FUSOU-APP を先行展開する（同時受理なし）
