...
`period_endpoint` は Supabase の `kc_period_tag` (カラム: `id`, `tag TIMESTAMPTZ`) の最新値を返す API です。値が更新されるとクライアント側の `asset_sync` ワーカーが内部キャッシュ (`PROCESSED_KEYS`) をクリアし、過去に送信済みのキーでも再アップロードを許可します。Cloudflare Pages 側は 1 日キャッシュなので、日次でタグを更新する運用を想定しています。
