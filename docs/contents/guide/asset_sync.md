...
2. API は Supabase アクセストークンを検証し、D1 のインデックス（および R2）に同名のオブジェクトが存在しないかをチェックします。問題がなければ `createSignedToken()` で HMAC-SHA256 署名済みトークンを生成し、120 秒間有効な `uploadUrl` を返します。レスポンスは `{ uploadUrl, expiresAt, fields }` 形式です。
...
| `key`                    | R2 に保存するオブジェクトキー。クライアントで固定プレフィックス `assets/` を付けた値を送信。 |
