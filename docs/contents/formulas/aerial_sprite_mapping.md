...
- `plane_from` に負値や範囲外インデックスが含まれていても、そのエントリは無視して続行される
...
### 4-4. 支援航空攻撃（Support Air）の友軍側例外

支援航空攻撃の友軍側は、通常航空戦の `plane_from` 基準とは異なる。

- 友軍側スプライトは `plane_from_f` ではなく、支援艦隊の艦種構成から決まる
- 判定対象の艦は、その戦闘レスポンスに含まれる支援艦隊 `api_ship_id` スナップショットを使う
- 対象艦種はクライアント実装で定義された空母系艦種集合
- 生成本数は対象艦の隻数を上限6で丸めた値
- 敵側は通常どおり `plane_from_e` と装備種別フィルタの経路を使う

したがって、Support Air の `friend sprite_total` は
通常航空戦と同じ式で再計算してはいけない。
