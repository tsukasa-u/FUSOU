...
echo ""
echo "# battle_files table structure"
echo "npx wrangler d1 execute fusou-battle-data --command \"PRAGMA table_info(battle_files);\""
echo ""
echo "# battle_files indexes"
echo "npx wrangler d1 execute fusou-battle-data --command \"SELECT sql FROM sqlite_master WHERE type='index' AND tbl_name='battle_files';\""
echo ""
echo "# Check if table_offsets column exists"
echo "npx wrangler d1 execute fusou-battle-data --command \"SELECT name FROM pragma_table_info('battle_files') WHERE name='table_offsets';\""
echo ""
echo ""
