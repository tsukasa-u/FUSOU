...
| Remote Result-race evidence | PASS | isolated evidence Worker exit `0`: one Direct invocation, Result PUT retained before rejected consume, no delete, retry returned `409 verification_retry_disabled`, and hashed job/trace telemetry correlated with the submitted values |
...
| Remote malicious callback corpus | PASS | Isolated evidence Worker run: all 10 forged callbacks returned bounded rejection outcomes; failed binding remained `not_verified`; consumed duplicate was accepted idempotently; mutated callback returned `422 verification_result_mismatch`; consumed binding remained verified |
| Remote mixed failure concurrency 4 | PASS | isolated evidence Worker exit `0`: failure, timeout, and two successes each used an independent random binding; every attempt had one Direct invocation, matching hashed telemetry, and zero input PUT/GET/DELETE operations |
| Remote mixed failure concurrency 8 | PASS | isolated evidence Worker exit `0`: failure and timeout remained `not_verified` with zero Result PUT; six successes were verified with one Result PUT and consumed authority state; all attempts had matching hashed telemetry and zero input PUT/GET/DELETE operations |
| Remote Direct evidence resource behavior | PASS | [`tlsn-remote-direct-no-input-r2-c1-c4-c8-final.json`](../../../packages/FUSOU-TLSN-VERIFICATION-WORKER/artifacts/tlsn-remote-direct-no-input-r2-c1-c4-c8-final.json): 65/65 samples completed and consumed; Result PUT and status Result reads matched every successful sample; input PUT/GET/DELETE were all zero and Result DELETE remained zero |
| Remote Direct latency regression target | PASS (evidence only) | Same artifact: client-visible p50/p95/p99/max were 1337.7/1599.4/1599.4/1599.4 ms at C=1, 1403.4/1835.6/2478.3/2478.3 ms at C=4, and 1362.9/1659.9/1980.1/1980.1 ms at C=8; every row was `MEASURED WITHIN TARGET` against the existing 3000 ms benchmark target |
| Direct Presentation byte fast path | PASS (local and evidence) | Direct binds the exact Presentation bytes in the authority claim and sends them through the Service Binding with signed metadata; it performs no initial input R2 PUT, verifier-side input R2 GET, or input cleanup DELETE. Presentation hash binding, Result PUT, Result SHA-256, and Durable Object consume remain authoritative; Queue/Trigger retain their R2 input path |
| Before/after Direct optimization | PASS (evidence comparison) | Before: client-visible p50 was 2421.8/2276.7/2238.7 ms at C=1/4/8, with 65 input PUTs and 65 observed input DELETEs. After: 1337.7/1403.4/1362.9 ms, with zero input PUT/GET/DELETE operations; Result PUT remained 65 and all 65 consumes completed |
| Direct phase and polling breakdown | PASS (evidence only) | After server-completion p50/p95/p99/max: C=1 956/1105/1105/1105 ms, C=4 945/1082/1158/1158 ms, C=8 904/1074/1119/1119 ms. Result persistence p50 was 871/860/837 ms and DO consume p50 was 24/26/24 ms. Status polling p50 was 1270.2/1290.7/1147.5 ms; client-observation p50 was 1270.3/1316.5/1240.0 ms |
| Result hash and telemetry contract | PASS (evidence only) | All 65 samples recorded the Result SHA-256 presence flag, complete `result_hash` timing stages, and matching hashed job/trace correlation; no raw identifiers or payload contents were present |
| Real Presentation payload scaling | NOT_ESTABLISHED | [`tlsn-remote-direct-payload-scaling.json`](../../../packages/FUSOU-TLSN-VERIFICATION-WORKER/artifacts/tlsn-remote-direct-payload-scaling.json) and the optimized artifact contain only available real sparse Presentations of approximately 3.5 KiB (3459-3461 bytes); all 4 KiB-8 MiB bands remain `NOT_ESTABLISHED`. No padding or arbitrary append was used. A real large `battle_data` Presentation corpus and request-transcript evidence are required before payload scaling can be promoted |
| Remote stale-attempt and lease fencing | PASS | [`tlsn-remote-stale-attempt.json`](../../../packages/FUSOU-TLSN-VERIFICATION-WORKER/artifacts/tlsn-remote-stale-attempt.json): lease 1000 ms, post-result pause 3000 ms, Result PUT 1, consume rejected, retained Result, late callback 1, stale attempt rejected, terminal `lease_expired`, retry 409, and input PUT/GET/DELETE all zero |
| Remote leakage audit | PASS | A local read-only audit over the before/after artifacts found no access token, device ID, private key, callback secret, auth JSON, raw job/trace/presentation fields, or callback material; after samples had zero timing correlation mismatches and zero input R2 operations |
...
```sh
pnpm exec dotenvx run --strict --overload -f packages/FUSOU-TLSN-VERIFICATION-WORKER/.env -fk packages/.env.keys -- env TLSN_REMOTE_BENCHMARK_WORKER_URL=https://fusou-tlsn-verification-evidence.ogu-hide-u-425.workers.dev TLSN_REMOTE_AUTH_MODE=test pnpm --dir packages/FUSOU-TLSN-VERIFICATION-WORKER exec node scripts/remote-direct-malicious-callback.mjs
```
...
The command was executed against the isolated evidence Worker. Its bounded output recorded all 10 forged callback outcomes; no report file containing callback bodies or secrets was created. This is evidence only for the evidence Worker and cannot establish a production SLO.

The isolated Direct evidence benchmark was run with:

```sh
pnpm exec dotenvx run --strict --overload -f packages/FUSOU-TLSN-VERIFICATION-WORKER/.env -fk packages/.env.keys -- env TLSN_REMOTE_BENCHMARK_WORKER_URL=https://fusou-tlsn-verification-evidence.ogu-hide-u-425.workers.dev TLSN_REMOTE_AUTH_MODE=test TLSN_REMOTE_EXPECTED_ENVIRONMENT=evidence TLSN_REMOTE_EXECUTION_MODE=direct TLSN_REMOTE_CONCURRENCY=1,4,8 TLSN_REMOTE_SAMPLE_COUNT=5 TLSN_REMOTE_CASES=p50 TLSN_REMOTE_BENCHMARK_REPORT_PATH="$PWD/packages/FUSOU-TLSN-VERIFICATION-WORKER/artifacts/tlsn-remote-direct-no-input-r2-c1-c4-c8-final.json" pnpm --dir packages/FUSOU-TLSN-VERIFICATION-WORKER exec node scripts/benchmark-tlsn-remote.mjs
```

The stale-attempt evidence was run against the same evidence Worker after an evidence-only deploy with `TLSN_TEST_VERIFICATION_LEASE_MS=1000` and `TLSN_TEST_POST_RESULT_DELAY_MS=3000`:

```sh
pnpm exec dotenvx run --strict --overload -f packages/FUSOU-TLSN-VERIFICATION-WORKER/.env -fk packages/.env.keys -- env TLSN_REMOTE_BENCHMARK_WORKER_URL=https://fusou-tlsn-verification-evidence.ogu-hide-u-425.workers.dev TLSN_REMOTE_AUTH_MODE=test TLSN_REMOTE_EXPECTED_ENVIRONMENT=evidence TLSN_REMOTE_EXPECTED_LEASE_MS=1000 TLSN_REMOTE_STALE_SETTLE_MS=3000 pnpm --dir packages/FUSOU-TLSN-VERIFICATION-WORKER exec node scripts/remote-direct-stale-attempt.mjs
```
...
The no-input-R2 Direct path was deployed and measured only on the isolated evidence Worker. The report is `packages/FUSOU-TLSN-VERIFICATION-WORKER/artifacts/tlsn-remote-direct-no-input-r2-c1-c4-c8.json`; it contains 65/65 successful samples across C=1/4/8, bounded request/source byte telemetry, complete Result hash timing, zero input PUT/GET/DELETE operations, and no raw identifier or payload leakage. The benchmark used cached real sparse fixtures without padding or arbitrary payload append. No canary or production binding/configuration is part of this change.

There was no separate production callback HTTP hop to remove. The optimization removes the Direct initial input R2 PUT, verifier-side Presentation R2 GET, and Direct input cleanup DELETE by carrying the original bytes in the existing Service Binding invocation. The signed metadata HMAC, Presentation hash binding, Result SHA-256, Result PUT, and Durable Object consume remain authoritative checks. Queue and Trigger continue to use their R2 input path.
