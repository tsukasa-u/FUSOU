...
pub fn get_tpm_quote(nonce: &[u8]) -> Result<TpmQuoteResult, AttestationError> {
    ...
    // 2. Primary Key 作成 (Endorsement Hierarchy)
...
**クレート**: `security-framework` (v2.x) + `core-foundation` (v0.10)
...
```typescript
// FUSOU-WEB src/server/utils/attestation-verifier.ts [NEW]
import type { TrustInput, AttestationLevel } from './trust-tag';

export interface AttestationReport {
  attestation_level: AttestationLevel;
  attestation_data?: string;          // base64 encoded
  ...
  environment?: {
    environment_type: string;         // "Native" | "VirtualMachine" | "Container" | "WineProton"
...
export async function verifyAttestation(
  ...
  if (!report) {
    return {
      ...
      environment_flags: { emulator_detected: false, debugger_detected: false, hook_detected: false },
      schema_fingerprint_valid: true,
    };
  }

  let attestation_valid = false;

  if (report.attestation_level === 'tpm' && report.attestation_data && report.public_key) {
    // TPM Quote 検証:
    // 1. AIK 公開鍵で Quote 署名を検証
    // 2. Quote 内の qualifyingData が nonce と一致するか確認
    // 3. PCR 値の基本的な妥当性チェック（既知の危険な値をブラックリスト照合）
    // Note: PCR値は OS更新で変動するため、ブラックリスト方式を使用（ホワイトリスト不可）
    attestation_valid = await verifyTpmQuote(report.attestation_data, report.public_key, nonce);
  } else if (report.attestation_level === 'secure_enclave' && report.attestation_data && report.public_key) {
    // Secure Enclave 署名検証:
    // 1. P-256 公開鍵で ECDSA 署名を検証
    // 2. 署名対象データが nonce と一致するか確認
    attestation_valid = await verifyEnclaveSignature(report.attestation_data, report.public_key, nonce);
  } else if (report.attestation_level === 'software_fingerprint') {
    // ソフトウェアフィンガープリントの場合は常に valid（フィンガープリント自体は検証ではなく情報収集）
    attestation_valid = !!report.fingerprint;
  }
  ...
  return {
    ...
    attestation_valid,
    environment_flags: {
      emulator_detected: report.environment?.environment_type !== 'Native' && report.environment?.environment_type !== undefined,
      debugger_detected: report.environment?.debugger_attached ?? false,
      hook_detected: (report.environment?.hooks_detected?.length ?? 0) > 0,
    },
    schema_fingerprint_valid: true, // 別途 Avro スキーマ検証で判定
  };
}

async function verifyTpmQuote(quotedB64: string, aikPubB64: string, nonce: string): Promise<boolean> {
  // WebCrypto API を使用して RSA-PSS 署名検証
  // 1. base64 → Uint8Array
  // 2. AIK 公開鍵を importKey
  // 3. crypto.subtle.verify('RSA-PSS', key, signature, data)
  // 4. Quote 内の qualifyingData と nonce を比較
  try {
    const quotedBytes = base64ToBytes(quotedB64);
    const aikPubBytes = base64ToBytes(aikPubB64);
    // ... 署名検証ロジック ...
    return true;
  } catch {
    return false;
...
async function verifyEnclaveSignature(sigB64: string, pubKeyB64: string, nonce: string): Promise<boolean> {
  // WebCrypto API を使用して P-256 ECDSA 署名検証
  // 1. base64 → Uint8Array
  // 2. P-256 公開鍵を importKey
  // 3. crypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, key, signature, data)
  try {
...
| ファイル | 種別 | 変更内容 |
|---------|------|---------|
| `FUSOU-APP/src-tauri/Cargo.toml` | MODIFY | `tss-esapi`, `sysinfo`, `security-framework`(macOS), `obfstr`, `[profile.release]` 設定追加 |
...
| `FUSOU-APP/src-tauri/src/attestation/secure_enclave_macos.rs` | NEW | macOS Secure Enclave P-256 ECDSA |
