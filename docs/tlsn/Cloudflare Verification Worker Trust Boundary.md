...
Status: local synthetic verification boundary with authenticated user ownership; device proof and production remain blocked
...
The Worker requires a Supabase Bearer access token on `/attestation/session` and `/verify/tlsn`. It resolves the token through Supabase `/auth/v1/user`, rejects anonymous users, and uses the returned `auth.users.id` as the canonical user subject. It issues a one-shot Session/Binding context at `/attestation/session` and accepts this strict JSON shape at `/verify/tlsn`:
...
The Worker does not yet establish device-level authority. It does not trust a client-supplied device ID or public key, and it is not connected to the existing FUSOU device registration, challenge, signature, and revocation path. User-level ownership is implemented; user-plus-device binding remains blocked.
...
The result field `notary_key_id` is therefore meaningful only when the registry lookup succeeds. It is not, by itself, a trust anchor. The result also contains `canonical_user_id`, which is generated from the authenticated Supabase user and included in the canonical JSON and Ed25519 signing bytes.
...
The Worker now uses `TlsnBindingAuthorityDurableObject`, one strongly consistent DO instance per SHA-256(binding value). The DO stores the Session ID, canonical user ID, nonce, binding, creation and expiry timestamps, status, used timestamp, and Presentation ID. Lookup performs request-time expiry checks and compares the authenticated user before verification; an alarm marks expired records, and a storage transaction makes consume single-use under concurrent requests. User, session, nonce, and consumed-binding conflicts return HTTP `409`.
...
| Authenticated user ownership | PASS, local test scope | Local tests reject missing/unknown credentials, reject cross-user binding use, and reject signed canonical-user mutation. Device proof remains blocked. |
...
| `TLSN_VERIFICATION_BOUNDARY` | PASS, local synthetic scope only | Cryptography, authenticated user ownership, and authority paths were exercised with synthetic evidence; production claims remain excluded. |
...
These checks establish synthetic local behavior only. They do not establish device proof, production Game Server authenticity, production operational key governance, production memory or payload limits, remote Worker behavior, or P0-05 evidence.
