...
>    `anon_sync_pepper_runtime` から `v_hash_version` を動的取得し、`member_id_hash_version` を記録する設計。Triple Owner Invariant (`member_ownership.verified_user_id` ≡ `user_devices.canonical_user_id` ≡ `user_member_map.user_id`) を維持する。  
