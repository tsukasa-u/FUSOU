...
> 10. **所有状態（`member_ownership`）と通常のアプリケーション経路で変更禁止な監査履歴（`member_identity_claims`）の分離**:  
>     現在の検証済み所有者レコードと、将来の監査検証用情報（`notary_time`, `notary_key_id`, `proof_purpose`, `request_range_start / request_range_length`, `response_range_start / response_range_length`）を含む Append-Only 監査証跡ログをテーブル分離する。  
> 11. **Quad Invariant の段階的成立 & Social User Binding**:  
>     `GAME_IDENTITY_VERIFIED` 時点で Triple Invariant を満たし、OAuth 認証ユーザーによる明示的なバインディング操作（`SOCIAL_ACCOUNT_BOUND`）完了後に $\text{member\_ownership.social\_user\_id} \equiv \text{user\_devices.canonical\_user\_id} \equiv \text{user\_member\_map.user\_id} \equiv \text{web\_user\_member\_map.user\_id}$ の Quad Invariant を厳格に保持する。  
...
> 2. 旧来のV2自己申告によるレガシー事前登録状態はランタイム上から**完全に削除**されます。過去に自己申告されたデータに対しては、`GAME_IDENTITY_VERIFIED` に達した本物のプレイヤーのClaimが到達した時点で、旧未検証Deviceを強制Revokeし、正規所有者へアトミックに上書き奪還します（レガシーリカバリ）。
...
2. **Challenge & Attestation Reuse Check**: 排他ロック下での Challenge 単一消費（`UPDATE claim_challenges SET challenge_status = 'CONSUMED',
    consumed_at = NOW()`）および `tlsn_attestation_id` 重複消費チェック。
...
8. **Audit Record Insert**: `member_identity_claims` への監査ログ追記（`proof_purpose = p_proof_purpose`, `tlsn_attestation_id`, `notary_time`, `notary_key_id`, `request_range_start / request_range_length`, `response_range_start / response_range_length` 含む。クライアント申告値は混入させない）。
9. **Device Verification Update**: 当該デバイスの `device_status = 'VERIFIED'` 昇格およびバインド更新。
...
5. **安全な Revoke 実行**: 同一 `public_id` に紐づく未検証端末のうち、**現在オーナーと異なる別ユーザーの端末のみを一括 Revoke**：
   ...
   ```
6. `member_identity_claims` に監査証跡ログを追記保存。
...
* **Attestation 一意消費制約 (Anti-Reuse)**:
  排他ロック取得後に `member_identity_claims` テーブルに `UNIQUE (tlsn_attestation_id)` 制約を課し、**同一の Attestation（Proof）が 2 回以上 Claim に使われた場合は `DUPLICATE_ATTESTATION_CLAIMED` 例外として即時ロールバック** します（**Presentation は何度生成されてもよいが、FUSOU Identity Claim は Attestation 単位で 1 度限り**）。
...
BEGIN
  ...
  -- Notary Time Verification (24時間以内の Proof かつ Clock Skew 5分許容)
  IF v_challenge.notary_time < NOW() - INTERVAL '24 hours' OR v_challenge.notary_time > NOW() + INTERVAL '5 minutes' THEN
    RAISE EXCEPTION 'TLSN_NOTARY_TIME_INVALID_OR_EXPIRED';
  END IF;
  
  -- public_id から api_member_id を逆引き (Challengeの発行元Identity)
  SELECT api_member_id INTO v_api_member_id FROM public.member_id_mapping WHERE public_id = v_challenge.public_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'ORPHANED_CHALLENGE: public_id mapping missing';
  END IF;
...
|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| 1 | Identity Spoofing (先回り登録) | 登録経路の物理的遮断 | `/anonymous-sync/` | - | TLSNotary `claim` / `social-bind` のみ | `register`, `pending`, `rpc_register_public_id`, `anonymous user` | `rpc_register_public_id`, `signInAnonymously` 参照箇所の完全削除, `anonymous-sync-v2.ts` ファイル自体の完全削除 | `test_client_supplied_member_id_rejected` |
...
- [D] `member_ownership`（現在状態）と `member_identity_claims`（REVOKE UPDATE, DELETE とトリガー trg_protect_member_claims_audit によりDBレベルで行の変更を物理禁止し、ON DELETE RESTRICTで親削除から守る真の Append-Only 監査履歴）の分離
