...
T3: 【Browser 同期完了】Proverのplaintext bufferからブラウザへ Response 返却 (Proof bufferとは分離)
...
T4: Notary が Session Header へ署名し Attestation 確定 -> Prover が custom range で Presentation 構築
...
| 7 | Legacy Token Bypass | 旧トークンの更新遮断 | Token Refresh Endpoint | - | Triple Verified User のみ | 旧 legacy token | JWT 検証側で `credential_version: 1` 必須化（is_verified 依存だけでなく Token schema レベルで拒絶） | `test_legacy_token_refresh_rejected` |
