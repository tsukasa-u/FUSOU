...
export async function listGoogleDriveFoldersWebClient(accessToken: string) {
  try {
    const response = await fetch(
      "https://www.googleapis.com/drive/v3/files?q=mimeType%3D%27application%2Fvnd.google-apps.folder%27%20and%20%27root%27%20in%20parents%20and%20trashed%20%3D%20false&corpora=user",
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
export async function check_file(
  ...
  folder_name: string,
  parent: string = "root",
  mime_type: string = "application%2Fvnd.google-apps.folder"
) {
  try {
    const response = await fetch(
      `https://www.googleapis.com/drive/v3/files?q=mimeType%3D%27${mime_type}%27%20and%20name%3D%27${folder_name}%27%20and%20%27${parent}%27%20in%20parents%20and%20trashed%20%3D%20false&corpora=user`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
export async function check_period(
  ...
  return check_file(
    ...
    folder_name,
    parent,
    "application%2Fvnd.google-apps.folder"
  );
}
